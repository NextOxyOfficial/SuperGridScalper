...
input double    BuyStopLossPips  = 40.0;   // Buy SL in pips (0 = no SL)
input double    SellStopLossPips = 40.0;   // Sell SL in pips (0 = no SL)
...
input int       GridSessionStartHour = 0;   // Asia window (UTC by default)
input int       GridSessionEndHour = 6;
input int       TrendSessionStartHour = 7;  // London/NY window (UTC by default)
input int       TrendSessionEndHour = 17;
input bool      EnableADXRegimeFilter = false;
input ENUM_TIMEFRAMES ADXTimeframe = PERIOD_M15;
input int       ADXPeriod = 14;
input double    MinADXForTrend = 22.0;
input double    MaxADXForGrid = 18.0;
input bool      EnableNewsBlackout = false;
input int       NewsBlackout1StartHourUTC = 12;
input int       NewsBlackout1EndHourUTC = 14;
input int       NewsBlackout2StartHourUTC = 18;
...
double g_CurrentADX = 0.0;            // Current ADX value
...
int OnInit()
{
    ...
    if(EnableADXRegimeFilter)
    {
        g_ADXHandle = iADX(_Symbol, ADXTimeframe, ADXPeriod);
        if(g_ADXHandle == INVALID_HANDLE)
            Print("WARNING: Failed to create ADX indicator handle");
    }
...
void OnTick()
{
    ...
    // Update ADX for regime filtering
    UpdateADX();

    // Detect sustained neutral market before placing new normal-mode entries
    UpdateNeutralMarketFilter();

    bool gridSessionActive = IsGridSessionActive();
    bool trendSessionActive = IsTrendSessionActive();
...
{
    g_CurrentADX = 0.0;
    if(!EnableADXRegimeFilter || g_ADXHandle == INVALID_HANDLE) return;

    double adxBuffer[];
    ArraySetAsSeries(adxBuffer, true);
    if(CopyBuffer(g_ADXHandle, 0, 0, 1, adxBuffer) < 1) return;

    g_CurrentADX = adxBuffer[0];
}
...
int GetSessionHour()
{
    ...
    int hour = tm.hour;
    if(UseUTCSessionClock)
        hour -= ServerToUTCOffsetHours;
    return NormalizeHour(hour);
}

bool IsHourWithinWindow(int hour, int startHour, int endHour)
{
...
bool IsGridSessionActive()
{
    if(!EnableSessionFilter)
        return true;
    return IsHourWithinWindow(GetSessionHour(), GridSessionStartHour, GridSessionEndHour);
}

bool IsTrendSessionActive()
{
    if(!EnableSessionFilter)
        return true;
    return IsHourWithinWindow(GetSessionHour(), TrendSessionStartHour, TrendSessionEndHour);
}
...
bool IsNewsBlackoutActive()
{
    ...
    int hourUtc = GetSessionHour();
    bool inWindow1 = IsHourWithinWindow(hourUtc, NewsBlackout1StartHourUTC, NewsBlackout1EndHourUTC);
    bool inWindow2 = IsHourWithinWindow(hourUtc, NewsBlackout2StartHourUTC, NewsBlackout2EndHourUTC);
    return (inWindow1 || inWindow2);
}

double GetSideRiskAmount(bool isBuy)
{
    if(!EnableSideRiskCap || MaxSideRiskPercent <= 0.0)
        return 0.0;
...
bool CanPlaceOrderWithinRiskCap(bool isBuy, double entryPrice, double slPrice, double lots)
{
    if(!EnableSideRiskCap || MaxSideRiskPercent <= 0.0)
        return true;
    if(slPrice <= 0.0)
    ...
    double newOrderRisk = 0.0;
    ENUM_ORDER_TYPE calcType = isBuy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
    double potential = 0.0;
    if(!OrderCalcProfit(calcType, _Symbol, lots, entryPrice, slPrice, potential))
        return false;
...
void ManageNormalGrid(bool isBuy)
{
    ...
    for(int i = 0; i < maxTargets && ordersPlaced < ordersNeeded; i++)
    {
        ...
        if(isBuy)
        {
            ...
            {
                AddToLog(StringFormat("BUY LIMIT skipped by risk cap @ %.2f | Lot: %.2f", targetPrice, lotToUse), "RISK");
                continue;
            }
        ...
        else
        {
            ...
            {
                AddToLog(StringFormat("SELL LIMIT skipped by risk cap @ %.2f | Lot: %.2f", targetPrice, lotToUse), "RISK");
                continue;
            }
...
void ManageRecoveryGrid(bool isBuy)
{
    ...
    if(totalPositionsThisSide < MaxRecoveryOrders && recoveryPendingCount == 0 && EnableRecovery)
    {
        ...
        if(!CanPlaceOrderWithinRiskCap(isBuy, recoveryPrice, recoverySL, recoveryLot))
        {
            AddToLog(StringFormat("%s recovery skipped by risk cap @ %.2f | Lot: %.2f",
                isBuy ? "BUY" : "SELL", recoveryPrice, recoveryLot), "RISK");
            return;
        }
